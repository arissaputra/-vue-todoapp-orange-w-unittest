      ...
      <form @submit.prevent="onSubmit" enctype="multipart/form-data">
        <div class="form-group">
          ...
          <template v-if="$v.form.description.$error">
            ...
            <div class="invalid-feedback" v-if="!$v.form.description.minLength">Description must have at least {{$v.form.description.$params.minLength.min}} letters.</div>
            <div class="invalid-feedback" v-if="!$v.form.description.maxLength">Description max length is {{$v.form.description.$params.maxLength.max}} letters.</div>
        ...
        <div class="form-group">
          ...
          <datepicker bootstrap-styling v-model="$v.form.deadline.$model" placeholder="Set up deadline"></datepicker>
          <template v-if="$v.form.deadline.$error">
            <div class="invalid-feedback" style="display: block" v-if="!$v.form.deadline.required">Field is required</div>
            <div class="invalid-feedback" style="display: block" v-if="!$v.form.deadline.minValue">Date should not be in the past</div>
        ...
        <div class="form-group" v-if="todo">
          ...
          <input accept="image/x-png,image/gif,image/jpeg" type="file" v-if="imageAccepted" class="form-control-file" ref="file" v-on:change="handleFileUpload()"/>
          <img class="img-thumbnail my-img" :src="imagePreview" v-show="showPreview"/>
          <img class="img-thumbnail my-img" :src="todo ? todo.snapshot:''" v-show="todo && todo.snapshot && !showPreview"/>
        </div>
        <div>
          <button :disabled="$v.$anyError || isLoading" type="submit" class="btn btn-primary float-right">
            <font-awesome-icon :icon="!isLoading ? 'save':'spinner'" :spin="isLoading" /> {{isLoading ? ' Submitting':' Save'}}
          </button>
  ...
  methods: {
    ...
    async onSubmit() {
      ...
      if (!this.$v.$invalid){
        ...
        if (this.todo) {
          if (this.snapshot) {
            const { success:snap_success, msg:snap_msg } = await this.uploadSnapshot()
            console.log(snap_success, snap_msg);
            
          }
          const { success, msg } = await this.updateTodo()
          console.log(success, msg);
          this.popUpResult(success)
